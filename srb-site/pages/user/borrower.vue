...
<script>
import borrowerApi from '~/api/borrower.js'
export default {
  data() {
    ...
    return {
      ...
      uploadUrl: BASE_API + '/srb/oss/uploadFile', //文件上传地址
    }
  },
  created() {
    this.getStatus()
  ...
  methods: {
    save() {
      ...
      borrowerApi.save(this.borrower).then((res) => {
        this.active = 1
      })
    },
    ...
    initDict() {
      ...
      this.getIncome()
      this.getIndustry()
      this.getReturnSource()
      this.getRelation()
    },
    getIndustry() {
      borrowerApi.listByDictCode('industry').then((res) => {
        this.industryList = res.data.dictList
      })
    },
    getEducation() {
      borrowerApi.listByDictCode('education').then((res) => {
        this.educationList = res.data.dictList
      })
    },
    getIncome() {
      borrowerApi.listByDictCode('income').then((res) => {
        this.incomeList = res.data.dictList
      })
    },
    getReturnSource() {
      borrowerApi.listByDictCode('returnSource').then((res) => {
        this.returnSourceList = res.data.dictList
      })
    },
    getRelation() {
      borrowerApi.listByDictCode('relation').then((res) => {
        this.contactsRelationList = res.data.dictList
      })
    },
    onUploadRemove(file, fileList) {
      //删除oss服务器上的内容
      borrowerApi.removeImg(file.response.data.url).then((response) => {
        // debugger
        console.log('远程删除')
    ...
    // 获取状态信息
    getStatus() {
      borrowerApi.getStatus().then((res) => {
