...
import borrowerInfoApi from '~/api/borrowerInfo.js'
export default {
  data() {
    return {
      active: null, //步骤
  ...
  methods: {
    initList() {
      this.getmoneyUseList()
      this.getreturnMethodList()
      this.checkStatus()
    },
    getreturnMethodList() {
      borrowerApi.listByDictCode('returnMethod').then((res) => {
        this.returnMethodList = res.data.dictList
      })
    },
    getmoneyUseList() {
      borrowerApi.listByDictCode('moneyUse').then((res) => {
        this.moneyUseList = res.data.dictList
      })
    },
    getBorrowCount() {
      borrowerInfoApi.getBorrowCount().then((res) => {
        this.borrowAmount = res.data.money
      })
    },
    save() {
      borrowerInfoApi.save(this.borrowInfo).then((res) => {
        this.$message({
          message: res.message,
          type: 'success',
        })
    ...
    checkStatus() {
      borrowerInfoApi.checkStatus().then((res) => {
        let status = res.data.status
        this.borrowInfoStatus = status
        if (status == 0) {
          this.active = 0
        } else if (status == 1) {
          this.active = 1
