...
import lendApi from '~/api/lend.js'
...
export default {
  ...
  mounted() {
    ...
    // 获取还款信息
    this.getLendReturnList()

    //获取回款计划
    this.getLendItemReturnList()
  },

  methods: {
    initDetail() {
      lendApi.show(this.$route.params.id).then((res) => {
        this.lend = res.data.lendDetail.lend
        this.borrower = res.data.lendDetail.borrower
      })
    },
    //查询账户余额
    fetchAccount() {
      lendApi.getCount().then((res) => {
        ...
        '前往汇付宝资金托管平台',
        {
          ...
          callback: (action) => {
            ...
            if (action === 'confirm') {
              ...
              lendApi.commitInvest(this.invest).then((response) => {
    ...
    // 获取投资记录列表
    getLendItem() {
      lendApi.getLendItem(this.$route.params.id).then((res) => {
        this.lendItemList = res.data.list
      })
    },
    // 获取还款计划列表
    getLendReturnList() {
      lendApi.getLendReturnList(this.$route.params.id).then((res) => {
        this.lendReturnList = res.data.list
      })
    },
    // 获取回款信息列表\
    getLendItemReturnList() {
      lendApi.getLendItemReturnList(this.$route.params.id).then((res) => {
    ...
    commitReturn(lendReturnId) {
      this.$alert(
        ...
        '前往汇付宝资金托管平台',
        {
          ...
          callback: (action) => {
            if (action === 'confirm') {
              lendApi.commitReturn(lendReturnId).then((response) => {
                document.write(response.data.formStr)
              })
            }
          },
