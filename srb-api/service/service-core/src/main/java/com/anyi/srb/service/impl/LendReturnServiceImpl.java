...
public class LendReturnServiceImpl extends ServiceImpl<LendReturnMapper, LendReturn> implements LendReturnService {
    ...
    public List<LendReturn> selectByLendId(Long lendId,Long userId) {
        ...
        queryWrapper.eq("lend_id", lendId).eq("user_id", userId);
    ...
    public String commitReturn(Long lendReturnId, Long userId) {
        ...
        //判断账号余额是否充足
        BigDecimal amount = userAccountService.getCount(userId);
        if (!(amount.doubleValue() >= lendReturn.getTotal().doubleValue())) {
