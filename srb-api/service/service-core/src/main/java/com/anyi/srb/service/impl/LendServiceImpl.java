...
import com.anyi.srb.enums.LendStatusEnum;
import com.anyi.srb.enums.ReturnMethodEnum;
...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    public void makeLoan(Long id) {
        ...
        UserInfo userInfo = userInfoMapper.selectById(userId);//给借款账号转入金额
    ...
    private void repaymentPlan(Lend lend) {
        ...
        for (int len = 1; len <= period; len++) {
            ...
            lendReturn.setCurrentPeriod(len);//当前期数
            lendReturn.setReturnMethod(lend.getReturnMethod());


            lendReturn.setFee(new BigDecimal(0));
            lendReturn.setReturnDate(lend.getLendStartDate().plusMonths(len)); //第二个月开始还款
