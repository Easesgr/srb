...
import user from "../../../../../srb-site/api/user";
export default {
  data() {
    return {
      list: null, // 数据列表
  ...
  methods: {
    getPage(page = 1) {
      this.page = page;
      userInfoApi.getList(this.page, this.limit, this.searchObj).then((res) => {
    ...
    lock(userId, status) {
      userInfoApi.changeStatus(userId, status).then((res) => {
        this.$message({
          type: "success",
          message: "更新成功!",
        });
    ...
    showLoginRecord(userId) {
      userInfoApi.getLoginLog(userId).then((res) => {
        this.loginRecordList = res.data.items;
